...
| Mehrere Anmeldungen auf demselben Gerät | Wenn Sie [!DNL Customer Attributes] zum Hochladen von Kundenprofilen in eine Datenquelle verwenden, empfiehlt Adobe, dass Benutzende dasselbe Gerät (d. h. dieselbe CX-Enterprise ID) verwenden. Die CX Enterprise ID (ECID) bleibt auf dem Gerät erhalten. Die gemeinsame Nutzung von Geräten kann dazu führen, dass die ECID mehrere Benutzer mit derselben ECID verknüpft, was zu unerwarteten Ergebnissen in [!DNL Target] führt. **Hinweis:** Bei Mobile ist die ECID dauerhaft, nachdem die Mobile App installiert wurde. Installieren Sie die App neu, um eine neue ECID zu generieren. Bei der Web-Version wird nach dem Löschen des Browser-Cookies eine neue ECID generiert. |
...
| Ist die Migration auf den CX Enterprise ID-Service erforderlich? | Die Migration hängt von den verwendeten Programmen ab. <ul><li>Adobe Analytics: dringend empfohlen </li><li>Adobe Target: erforderlich. </li></ul>Der CX Enterprise ID-Service ermöglicht die neuesten CX Enterprise-Funktionen, einschließlich Echtzeit-Zielgruppen, Adobe Target-Modernisierung, Analytics-Integration und Video-Heartbeat-Tracking. |
