...
## Implementieren des Besucher-ID-Service
...
Wenn Sie [!UICONTROL Experience Platform-Tags] nicht verwenden, implementieren Sie den Besucher-ID-Dienst manuell über die JavaScript-Bereitstellung (`VisitorAPI.js`) wie folgt:
...
* Rufen Sie `visitor.setCustomerIDs({"crm_id":"52mc210tr42"})` mithilfe des Besucher-ID-Service auf. Oder
...
Verwenden Sie den folgenden Prozess, um sicherzustellen, dass der Besucher-ID-Service auf Ihrer Site korrekt implementiert ist.

1. Löschen Sie die Cookies für Ihre Site, damit Sie die Anfrage an den Besucher-ID-Dienst sehen können (die Anfrage erfolgt beim ersten Besuch und dann einmal pro Besucher und Woche).
...
1. Stellen Sie sicher, dass die Analytics-Anfrage den `mid` (die ECID) enthält. Während der Übergangsphase (sofern aktiviert) sollte auch ein `aid`-Parameter angezeigt werden (die Analytics-Besucher-ID).

Erwartete Antwort mit enthaltener ECID:

![Erwartete Antwort mit enthaltener ECID](../assets/mac_id_response.png)

Analytics-Bildanforderung mit enthaltener ECID (auch bekannt als `mid` oder _Besucher-ID_):

![Analytics-Bildanforderung mit enthaltener ECID](../assets/mid.png)

ECID in der mbox-Anfrage:

![ECID in der mbox-Anfrage](../assets/mbox_request.png)

### Was ist die Übergangsphase?

Nach der Bereitstellung des Besucher-ID-Service erhalten neue Besucher keine Analytics-ID mehr (`aid`). Wenn in bestimmten Bereichen Ihrer Site der Besucher-ID-Service noch nicht implementiert wurde und Besucher diese Bereiche aufrufen, wird die ECID (`mid`) nicht erkannt und den Besuchern wird eine Legacy-Analytics-ID (`aid`) zugewiesen. Diese Einrichtung kann potenzielle Probleme verursachen, einschließlich doppelter Besuche und falscher Attribution.

Wenn beispielsweise der Supportbereich Ihrer Site in einem separaten CMS verwaltet wird, haben Sie möglicherweise eine andere Analytics-JavaScript-Datei für diesen Bereich. Wenn Sie die ECID auf Ihrer Haupt-Site bereitstellen, bevor Sie den Besucher-ID-Service auf der Support-Site bereitstellen, erhalten neue Besucher beim Besuch des Support-Abschnitts eine alte Analytics-ID. Besuche, die beide Site-Bereiche umfassen, werden als unterschiedliche Besuche gemeldet.

Die Bereitstellung des Besucher-ID-Service auf Sites, die mehrere JavaScript-Dateien oder andere Technologien (wie Flash) verwenden, kann Koordinierungsprobleme verursachen. Diese Probleme treten auf, da Sie den Besucher-ID-Dienst für alle Teile Ihrer Site gleichzeitig aktivieren müssen. Durch das Konfigurieren einer Übergangsphase können neue Besucher weiterhin eine Analytics-Besucher-ID vom Besucher-ID-Service erhalten. Besucher können konsistent in Bereichen Ihrer Site identifiziert werden, die nicht auf die Verwendung des Besucher-ID-Service aktualisiert wurden.
...
* (Optional) Eine oder mehrere Ihrer IDs. Abhängig von Ihrer Implementierung des Besucher-ID-Service können Sie auch eine oder mehrere Ihrer IDs senden, z. B. CRM-IDs oder Hash-E-Mail-Adressen. Wenn diese Daten an Adobe Analytics gesendet werden, werden sie an Adobe Audience Manager übertragen. Adobe empfiehlt, Adobe Analytics keine personenbezogenen Daten bereitzustellen. Verwenden Sie stattdessen einen unidirektionalen Hash, um die Daten zu maskieren, bevor sie an Adobe gesendet werden.
* Segmente aus [!DNL Analytics] über die Back-End-Funktion zur Segmentfreigabe.
* Das `demdex.net`-Cookie wird gesetzt, wenn keine Drittanbieter-Cookies blockiert werden. Das `AMCV_###@AdobeOrg` Erstanbieter-Cookie wird immer mit dem Besucher-ID-Dienst festgelegt.
