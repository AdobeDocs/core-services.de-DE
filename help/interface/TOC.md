---
role: User,Admin
user-guide-title: Handbuch für die zentrale Benutzeroberfläche und Administration von CX Enterprise
breadcrumb-title: CX Enterprise-Oberfläche und -Administration
title: CX Enterprise-Benutzeroberfläche und Administrationshandbuch
description: Erfahren Sie mehr über die Benutzeroberfläche und Administration von CX Enterprise. Durchsuchen Sie Themen zu KI, Administration, Shared Services, Datenerfassung, Cookies, Versionshinweisen und FAQs.
index: true
source-git-commit: f8271b21134641c4576abc8a7e1fb8874ae68350
workflow-type: tm+mt
source-wordcount: '210'
ht-degree: 59%

---

# CX Enterprise-Oberfläche und -Administration {#interface}

+ [CX Enterprise-Oberfläche und -Administration](/help/interface/experience-cloud.md)
+ KI in CX Enterprise {#features}
   + [Generative KI](/help/interface/features/generative-ai.md)
   + [Agent-basierte KI](features/agentic-ai.md)
   + {hide-from-toc}[AI-Überwachung](/help/interface/features/monitoring.md)
   + [Agent-Vorgänge und KI-Kreditverbrauch](/help/interface/features/ai-credit-consumption.md)
+ Administration {#administration}
   + [Kontovoreinstellungen und Benachrichtigungen](/help/interface/features/account-preferences.md)
   + [Browser- und Sprachunterstützung](/help/interface/browser-language.md)
   + [Benutzer- und Produktverwaltung](/help/interface/administration/admin-console.md)
   + [Admin-Tool](/help/interface/administration/admin-tool-experience-cloud.md)
   + [Organisationen und Kontoverknüpfung](/help/interface/administration/organizations.md)
   + [Integrationen](/help/interface/administration/integrations.md)
   + [Einheitliche Suche](/help/interface/features/search.md)
   + [Voreinstellungen für personalisierte Lerninhalte](/help/interface/features/personalized-learning.md)
+ Schnittstellendienste {#services}
   + [Übersicht über die Benutzeroberfläche](/help/interface/services/overview.md)
   + [Erste Schritte](/help/interface/services/getting-started.md)
   + Assets {#assets}
      + [Übersicht über Assets](/help/interface/services/assets/experience-cloud-assets.md)
      + [Verwaltung der Creative Cloud-Benutzenden](/help/interface/services/assets/manage-cc-users.md)
      + [Asset-Ordner freigeben](/help/interface/services/assets/share.md)
      + [Hochladen eines Assets in einen freigegebenen Ordner](/help/interface/services/assets/upload.md)
      + [Dateivoraussetzungen für die Freigabe von Assets](/help/interface/services/assets/file-reqs.md)
      + [Verhalten freigegebener Ordner](/help/interface/services/assets/behavior.md)
   + Zielgruppenbibliothek {#audiences}
      + [Zielgruppen – Überblick](/help/interface/services/audiences/overview.md)
      + [Erstellen einer Zielgruppe](/help/interface/services/audiences/create.md)
   + Kundenattribute {#customer-attributes}
      + [Übersicht über Kundenattribute](/help/interface/services/customer-attributes/attributes.md)
      + [Häufig gestellte Fragen](/help/interface/services/customer-attributes/faq-crs.md)
      + [Datendateien und -quellen](/help/interface/services/customer-attributes/crs-data-file.md)
      + [Erstellen und Hochladen von Daten](/help/interface/services/customer-attributes/t-crs-usecase.md)
      + [Optional – Hochladen per FTP](/help/interface/services/customer-attributes/t-upload-attributes-ftp.md)
      + [Schema prüfen](/help/interface/services/customer-attributes/validate-schema.md)
      + [Konfigurieren von Abonnements](/help/interface/services/customer-attributes/subscription.md)
      + [Schema aktualisieren](/help/interface/services/customer-attributes/t-update-schema.md)
      + [Unterstützung für die DSGVO](/help/interface/services/customer-attributes/gdpr.md)
      + [Unterstützung für den CCPA](/help/interface/services/customer-attributes/ccpa.md)
      + [Überlegungen zum Datenschutz](/help/interface/services/customer-attributes/privacy-mac.md)
   + [Trigger](/help/interface/services/triggers.md)
   + [Adobe Exchange](/help/interface/services/exchange.md)
+ Datenerfassung {#data-collection}
   + [Adobe Managed Certificate Program](/help/interface/data-collection/adobe-managed-cert.md)
   + [IP-Adressen](/help/interface/data-collection/ip-addresses.md)
   + [Domains](/help/interface/data-collection/domains.md)
   + Cookies {#cookies}
      + [Übersicht über Cookies](/help/interface/data-collection/cookies/overview.md)
      + [Cookies in Advertising](/help/interface/data-collection/cookies/advertising.md)
      + [Cookies in Analytics](/help/interface/data-collection/cookies/analytics.md)
      + [Cookies in Audience Manager](/help/interface/data-collection/cookies/audience-manager.md)
      + [Cookies in CX Enterprise](/help/interface/data-collection/cookies/experience-cloud.md)
      + [Cookies in Target](/help/interface/data-collection/cookies/target.md)
      + [Cookies im Web SDK](/help/interface/data-collection/cookies/web-sdk.md)
   + [Regionale Datenerfassung](/help/interface/data-collection/rdc.md)
   + [Verwenden des DNS-Vorabrufs](/help/interface/data-collection/dns-prefetch.md)
+ Versionshinweise und Ressourcen {#more-resources}
   + [Versionshinweise](/help/interface/more-resources/release-notes.md)
   + [Dokumentation – Aktualisierungen](/help/interface/more-resources/doc-updates.md)
   + [Häufig gestellte Fragen](/help/interface/more-resources/faq.md)
   + [Terminologie](/help/interface/more-resources/terms.md)
